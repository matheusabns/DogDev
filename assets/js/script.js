...
let menuOpner = document.querySelector('.menu-opner');
let nav = document.querySelector('header nav');

menuOpner.addEventListener('click', () => {
    if (nav.classList.contains('opened')) {
        nav.classList.remove('opened');
        menuOpner.querySelector('.close-icon').style.display = 'none';
        menuOpner.querySelector('.hamburguer-icon').style.display = 'flex';
    } else {
        nav.classList.add('opened');
        menuOpner.querySelector('.close-icon').style.display = 'flex';
        menuOpner.querySelector('.hamburguer-icon').style.display = 'none';
    }
})
...
let testimonials = [
    ...
    { quote: '"DogDev é um serviço de entrega direta ao consumidor, preparado na hora com ingredientes 100% reais. Ingredientes que os humanos reconheceriam."', origin: 'forbes.svg' },
    { quote: '"Vejo voce[João] como um verdadeiro herói"', origin: 'sharktank.svg' },
...
for (let tindex in testimonials) {
    ...
    img.setAttribute('src', './assets/img/' + testimonials[parseInt(tindex)].origin);
